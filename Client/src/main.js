...
const WelcomeAPI = (welcomeID, welcomeBodyID) => {
    $.ajax({
       cache: false,
       type: 'GET',
...
const SubmitPost = (id, peID, cppID) => {
    $(id).on('click', () => {
        let post = `"${$('#name').val()}"`;
        JSON.stringify(post)
...
let welcome = "<div id='welcome-res'>Loading...</div>";
let welcomeBody = "<div id='welcome-body'></div>";
